...
- `app/ui/container/` — a FastAPI + React app; `backend/main.py` does **not** call the Query Lambda — it queries a Bedrock Knowledge Base (`KNOWLEDGE_BASE_ID` in `.env.local`) in two steps: `bedrock-agent-runtime` `retrieve` for chunks, then `bedrock-runtime` `converse` (with the guardrail in `GUARDRAIL_ID`/`GUARDRAIL_VERSION` — ID/ARN, not name — when set; sources are suppressed when it intervenes) to write the answer. RetrieveAndGenerate is unsupported for managed KBs. The UI date range is authoritative and is enforced twice: a `date_numeric` (YYYYMMDD int, from the scraper's `.metadata.json` sidecars) metadata filter is passed to `retrieve` via `vectorSearchConfiguration` — a KB whose vector store indexes sidecar attributes (e.g. an S3 Vectors-backed KB) enforces it natively — and results are then post-filtered against any date attribute they carry; undated chunks are dropped whenever a range is set. `_retrieve` degrades across KB types (vector+filter → vector unfiltered, deeper → `managedSearchConfiguration` → bare) and caches the strategy that worked. Sources are returned as `{label, url}`: CSMS message text links to its public GovDelivery bulletin (ID in hex, derived from the KB's S3 location), archive-derived pages to their fetched URL, and any other S3 object to a presigned link (`SOURCE_URL_TTL_SECONDS`). Built as a Docker image and run **locally only** via `docker-compose.yml`; the matching Fargate deployment (`resource_fargate.py`) is fully wired up but commented out in `rag_stack.py` — UI is not deployed to Fargate.
