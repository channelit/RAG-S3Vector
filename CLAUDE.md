...
- `app/ui/container/` — a FastAPI + React app; `backend/main.py` does **not** call the Query Lambda — each question is one `bedrock-agent-runtime` `retrieve_and_generate` call against a Bedrock Knowledge Base (`KNOWLEDGE_BASE_ID` in `.env.local`, model in `BEDROCK_MODEL_ARN`, guardrail via `GUARDRAIL_ID`/`GUARDRAIL_VERSION` — ID, not name — as `generationConfiguration`; sources are suppressed when `guardrailAction` is `INTERVENED`). There is no custom system prompt and no S3 access. **RetrieveAndGenerate is rejected by managed KBs** ("not supported for managed knowledge bases") — the KB must be a standard one (e.g. S3 Vectors-backed). The UI date range is enforced in exactly one place: a `date_numeric` (YYYYMMDD int, from the scraper's `.metadata.json` sidecars) metadata filter in the retrieval configuration, applied natively by the KB's vector store — no post-filter, no fallback, no other date source. Sources come from the response citations as `{label, url}`: CSMS message text links to its public GovDelivery bulletin (ID in hex, parsed from the cited document path), attachments to their sidecar `attachment_url` when the KB exposes sidecar metadata. Self-hosted ALTCHA proof-of-work captcha (on by default, `ALTCHA_ENABLED=false` turns it off; there is no separate ALTCHA server): `/api/config` tells the frontend whether it's on, and the widget fetches challenges from the relative path `/api/altcha/challenge`, where the backend creates them with the `altcha` PyPI lib. `/api/query` requires an `altcha` payload and verifies it with the same lib and `ALTCHA_HMAC_KEY` (signature + solution + expiry + in-memory single-use guard); when the key is unset a random per-process key is used, which only works for a single worker/replica. The frontend imports the `altcha` npm web component and disables Submit until verified. Built as a Docker image and run **locally only** via `docker-compose.yml`; the matching Fargate deployment (`resource_fargate.py`) is fully wired up but commented out in `rag_stack.py` — UI is not deployed to Fargate.
...
Runs the FastAPI/React UI locally against real AWS (Bedrock Knowledge Base, Bedrock, Guardrails) — no local mocking, LocalStack is not supported (doesn't implement Bedrock/S3 Vectors).

```bash
cd app/ui/container
cp .env.local.example .env.local     # set KNOWLEDGE_BASE_ID, AWS_PROFILE (optionally GUARDRAIL_ID)
